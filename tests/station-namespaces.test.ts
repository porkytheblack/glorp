...
import { loadStationConfig, type StationConfig } from "../src/station/config.ts";
...
import {
  ...
  slugify,
} from "../src/station/namespace-store.ts";
import { selectNamespaceId, NamespaceForbiddenError } from "../src/station/auth/middleware.ts";
import { NamespaceCredentialsStore } from "../src/station/credentials.ts";
import { CredentialsStore } from "../src/agent/credentials.ts";
import type { ApiKey } from "../src/station/auth/types.ts";
...
describe("NamespaceCredentialsStore", () => {
  it("reads its own file first, then falls back to the station store", () => {
    const stationDir = tmp();
    const nsDir = tmp();
    const station = new CredentialsStore(stationDir);
    station.upsertProvider({ type: "known", id: "anthropic", apiKey: "station-key" });
    const ns = new NamespaceCredentialsStore(nsDir, station);
    // Miss in the namespace file → station fallback.
    expect(ns.getProvider("anthropic")?.apiKey).toBe("station-key");
    ...
    expect(station.getProvider("anthropic")?.apiKey).toBe("station-key"); // unchanged
  });

  it("disables the fallback when the base points at the same file (default ns)", () => {
    const dir = tmp();
    const station = new CredentialsStore(dir);
    const same = new NamespaceCredentialsStore(dir, station);
...
async function startAuthed(): Promise<{ base: string; admin: string; config: StationConfig }> {
  ...
  return { base: `http://127.0.0.1:${station.port}`, admin, config };
}
...
describe("namespace HTTP surface", () => {
  ...
  it("keeps the default namespace on the legacy layout (back-compat)", async () => {
    const config = loadStationConfig({ hostname: "127.0.0.1", port: 0, dataDir: tmp() });
    const station = await startStation(config);
    stations.push(station);
    const call = caller(`http://127.0.0.1:${station.port}`);
