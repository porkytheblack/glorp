...
import { isAllowedBrowserOrigin, startGarage } from "../src/garage/server.ts";

const tmpDirs: string[] = [];

function tmp(prefix = "garage-fix-"): string {
...
describe("Garage snapshot rehydration", () => {
...
describe("Garage browser origin checks", () => {
  it("allows same-origin, loopback dev, and non-browser clients only", () => {
    ...
    expect(isAllowedBrowserOrigin("http://localhost:5173", garageUrl)).toBe(true);
  ...
  it("rejects cross-site REST requests before routing", async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${garage.port}/sessions`, {
