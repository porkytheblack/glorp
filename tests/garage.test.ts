/**
 * Glorp Garage backend tests. These exercise everything that doesn't require
...
import { EventStream, type StreamClient } from "../src/garage/event-stream.ts";
import { GarageSession } from "../src/garage/session.ts";
import { SessionManager } from "../src/garage/manager.ts";
import { SessionCredentialsStore } from "../src/garage/credentials.ts";
import { interpolate, provision } from "../src/garage/templates/engine.ts";
...
function tmp(prefix = "garage-test-"): string {
...
function makeSession(id: string, dataDir: string): GarageSession {
  return new GarageSession({ id, workspace: path.join(dataDir, id), dataDir, permissionMode: "normal" });
}
...
describe("loadGarageConfig", () => {
  it("layers defaults < garage.json < overrides", () => {
    const dataDir = tmp();
    fs.writeFileSync(path.join(dataDir, "garage.json"), JSON.stringify({ port: 9999 }));
    const base = loadGarageConfig({ dataDir });
    expect(base.port).toBe(9999);
    expect(base.hostname).toBe("127.0.0.1");
    const overridden = loadGarageConfig({ dataDir, port: 5000 });
...
describe("HTTP surface (integration)", () => {
  it("serves health, session CRUD, state, models and rehydration", async () => {
    ...
    try {
      ...
      expect(created.body.ws_url).toContain(`:${garage.port}/api/v1/sessions/`);
