...
describe("Model endpoints", () => {
  async function withStation(fn: (call: (m: string, p: string, b?: unknown) => Promise<{ status: number; body: any }>) => Promise<void>) {
    const dataDir = tmp();
    const station = await startStation(loadStationConfig({ dataDir, port: 0, hostname: "127.0.0.1" }));
