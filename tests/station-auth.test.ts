...
import { FileKeyStorage } from "../src/station/auth/file-key-storage.ts";
...
describe("server auth enforcement", () => {
  async function startAuthed(scopes: string[] = ["admin"]) {
    ...
    return { base: `http://127.0.0.1:${station.port}`, key };
  }
  ...
  it("is open with no key on a loopback bind (auto default)", async () => {
    ...
    const station = await startStation(config);
    stations.push(station);
    expect((await fetch(`http://127.0.0.1:${station.port}/sessions`)).status).toBe(200);
  });
});
