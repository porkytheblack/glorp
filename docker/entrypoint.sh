#!/usr/bin/env bash
# Boot Glorp Garage. On first start (no keys yet) auto-mint an admin API key and
# print it to the logs, so the container is usable out of the box. Set
# GLORP_AUTO_KEY=0 to disable, or GLORP_GARAGE_AUTH=off to run without auth.
set -euo pipefail
...
if [ "${1:-}" != "keys" ] \
  && [ "${GLORP_GARAGE_AUTH:-required}" != "off" ] \
  && [ "${GLORP_AUTO_KEY:-1}" = "1" ] \
