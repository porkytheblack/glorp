...
/**
 * Glove's `Context` class is private; we hand the live store + a few thin
 * wrappers to anything that needs a Context-shaped object (the fleet's
 * inbox resolver, the task/inbox tool factories).
 */
export function makeInboxContext(store: GlorpStore): Context {
  return {
    store,
    getMessages: () => store.getMessages(),
    appendMessages: (m: Parameters<typeof store.appendMessages>[0]) => store.appendMessages(m),
    ...
    addInboxItem: (i: Parameters<typeof store.addInboxItem>[0]) => store.addInboxItem(i),
    updateInboxItem: (
