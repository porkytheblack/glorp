...
export const CATEGORIES: readonly DeliverableCategory[] = [
  {
    ...
    clearedBy: new Set<ClearSignal>(["browser", "reviewer", "reread"]),
    header: "Unvalidated web/UI (serve and look at it before claiming completion):",
    steps: [
      "Serve or open the page and actually look at it — do not claim a UI works from source alone.",
      "Drive it (playwright/puppeteer/cypress) or screenshot it; check layout, responsive behavior, text overflow, interactive/empty/error states, and asset rendering on desktop and mobile widths.",
  ...
  },
  {
    ...
    header: "Unvalidated presentation (render and review before claiming completion):",
    steps: [
      "Run the skill's validator if one exists (e.g. `scripts/office/validate.py`); a deck that opens is not the same as a good deck.",
      "Render/re-read the slides and judge them: per-slide message, no overflow off the canvas, consistent layout/typography, no placeholder/lorem content.",
