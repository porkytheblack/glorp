...
export function taskWorkerPreamble(taskType: string, deliverable?: DeliverableContract | null): string {
  const lines = [
    ...
    `When the work is done, call **deliver_result** with a short summary and the paths of every deliverable file (place files in ./uploads or ./output — they are made available to the requester automatically). A task that calls for several documents delivers them ALL in that one call; list only the finished artifacts, never your generator scripts, drafts, or logs. Calling it again after a follow-up change replaces the previous result.`,
