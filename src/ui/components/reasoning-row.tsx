...
export function ReasoningRow({ text }: { text: string }) {
  ...
  return (
    <box flexDirection="column" marginBottom={1} paddingLeft={2}>
      <text fg={REASONING_BORDER}>
        {"--- reasoning trace ---"}
      </text>
      {preview.map((line, i) => (
        <text key={i} fg={REASONING_COLOR}>
          {line || " "}
        </text>
      ))}
      <text fg={REASONING_BORDER}>
...
export function ReasoningToggleHint({ active }: { active: boolean }) {
  return (
    <text fg={active ? theme.accent : theme.textDim}>
      {active ? "[R]" : ""}
    </text>
  );
}
