...
export function MessageRow({ turn, showReasoning }: { turn: ChatTurn; showReasoning?: boolean }) {
  ...
  if (turn.kind === "tool" && turn.tool) {
    return <ToolCallRow tool={turn.tool} />;
  }
  const text = turn.text ?? "";
  const imgCount = (turn.meta?.imageCount as number) || 0;
  return (
    <box flexDirection="row" marginBottom={1}>
      <box width={6} marginRight={1}>
        <text fg={LABEL_COLOR[turn.kind]}><strong>{LABEL[turn.kind]}</strong></text>
      </box>
      <box flexDirection="column" flexGrow={1}>
        {showReasoning && turn.reasoning && <ReasoningRow text={turn.reasoning} />}
        {imgCount > 0 && <text fg={theme.accent}>{"📎 "}{imgCount}{" image"}{imgCount > 1 ? "s" : ""}</text>}
        {text.split("\n").map((line, i) => (
...
function isOrchestratorTurn(turn: ChatTurn): boolean {
  return turn.kind === "system" && (turn.meta?.orchestrator as boolean) === true;
}

function OrchestratorRow({ turn }: { turn: ChatTurn }) {
  const text = turn.text ?? "";
  return (
    <box flexDirection="row" marginBottom={1}>
      <box width={6} marginRight={1}>
