/**
 * Idle-session garbage collector. A long-running Station accumulates loaded
 ...
 */
export function startIdleGc(registry: NamespaceRegistry, config: StationConfig): () => void {
  const ttl = config.idleSessionTtlMs;
  if (ttl <= 0) {
    console.log("[glorp-station] idle-session GC: disabled (idleSessionTtlMs=0)");
    return () => {};
  }
  console.log(
    `[glorp-station] idle-session GC: on (ttl ${Math.round(ttl / 1000)}s, every ${Math.round(config.gcIntervalMs / 1000)}s)`,
  );
...
async function sweep(registry: NamespaceRegistry, ttl: number): Promise<void> {
  for (const bundle of registry.liveBundles()) {
    try {
      ...
      if (reaped.length > 0) {
        console.log(
          `[glorp-station] gc: unloaded ${reaped.length} idle session(s) in ${bundle.ns.id}`,
        );
      }
    } catch (err) {
      console.error(`[glorp-station] gc sweep failed for ${bundle.ns.id}:`, err);
    }
  }
}
