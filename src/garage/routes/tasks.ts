...
export function taskRoutes(
  ...
  return {
    ...
    async create(req): Promise<Response> {
      ...
      if (!prompt || typeof prompt !== "string") return errorJson("bad_request", "Missing 'input.prompt'", 400);

      const id = newSessionId();
      const created_at = new Date().toISOString();
      const record = store.create({ id, type, created_at, ...(body.callback_url ? { callback_url: body.callback_url } : {}) });
