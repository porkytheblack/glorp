/**
 * On-disk session snapshot helpers used for rehydration. Garage reuses the
 * agent's `GlorpStore` snapshot files so a session created in a previous
 * Garage process can be listed and resumed.
 */
...
/** Read just the metadata Garage needs to rehydrate a session, or null. */
