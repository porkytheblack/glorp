 ...
 * over the garage's default credentials as a fallback.
 *
 ...
 * reads delegate to the namespace store (then the garage fallback) except the
 ...
 *   session custom key  >  namespace credentials  >  garage credentials
 */
 ...
 * `<nsDataDir>/credentials.json`, but on a read miss falls back to the garage's
...
export class SessionCredentialsStore extends NamespaceCredentialsStore {
  ...
  /** Drop the custom credential. Returns the garage profile id to revert to. */
  ...
  /**
   * Active namespace profile (falling back to the garage default), ignoring the
   * session custom-key overlay. Used to revert when a custom key is cleared.
   */
  garageDefaultProfileId(): string | undefined {
